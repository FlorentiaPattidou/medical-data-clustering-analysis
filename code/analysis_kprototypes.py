# ============================================================
# patients_analysis_kprototypes.py
# KPrototypes (k=2) + PLOTS + EVALUATION for ALL PATIENTS
#
# Runs full analysis into: outputs_kproto/
#
# Produces:
#   01_pca_explained_variance.png
#   02_elbow_kproto_cost.png
#   03_quality_metrics_vs_k.png
#   04_cluster_sizes.png
#   05_pca_clusters_with_centers_status.png
#   07_<feature>_by_cluster.png  (categorical)
#   08_<feature>_by_cluster.png  (numerical boxplots)
...
class Config:
    ...
    base_dir: str = "outputs_kproto"

    # categorical features
    cat_features: Tuple[str, ...] = ("stenosis", "gsm", "plarea", "dwa", "ctiastr", "jba")

    # numerical features (all available)
    ...
    km_init: str = "Huang"
    km_n_init: int = 20
    gamma: float = 0.5   # δοκίμασε: 0.1, 0.5, 1.0, 2.0, 5.0
...
def prepare_data(
    ...
    """
    KPrototypes expects a single array with numericals first,
    then categoricals. Returns array + cat_indices.
    Returns only rows with no missing values in selected features.
    """
    all_features = list(num_features) + list(cat_features)
    df_sub = df[all_features].dropna()

    # scale numericals
    scaler = RobustScaler()
    X_num = scaler.fit_transform(df_sub[list(num_features)].values.astype(float))

    # categoricals as object
    X_cat = df_sub[list(cat_features)].astype(str).values

    X = np.hstack([X_num, X_cat])
    cat_indices = list(range(len(num_features), len(all_features)))

    return X, cat_indices, df_sub.index.tolist(), scaler
...
def save_num_distributions(
    ...
    for feat in features:
        fig, ax = plt.subplots(figsize=figsize)
        groups = [
            data_out.loc[data_out["Cluster"] == cid, feat].dropna().values
            for cid in sorted(data_out["Cluster"].unique())
        ]
        ax.boxplot(groups, tick_labels=[f"Cluster {cid}" for cid in sorted(data_out["Cluster"].unique())])
...
def save_pca_plot(
    ...
    STATUS_OFFSET = {"sympt": np.array([0.04, 0.0]), "asympt": np.array([-0.04, 0.0])}
    STATUS_CFG = {
    ...
    for cid in range(k):
        for status, scfg in STATUS_CFG.items():
            idx = (clusters == cid) & (patstat == status)
            if not np.any(idx):
                continue
            pts = Z[idx].copy()
            ...
            ax.scatter(pts[:, 0], pts[:, 1],
                       ...
                       marker=scfg["marker"], s=scfg["size"],
                       alpha=scfg["alpha"],
                       edgecolors=scfg["edge"], linewidths=scfg["lw"],
                       zorder=5)
    ...
    for cid in range(k):
        ...
        ax.scatter(cx, cy, marker="*", s=500, zorder=10,
                   color=CLUSTER_COLORS[cid], edgecolors="black", linewidths=1.5)
    ...
    ax.set_title("PCA 2D projection (KProto)  |  color = cluster    marker = patient status",
                 fontsize=11, pad=10)
    ax.set_xlabel("PC1")
    ax.set_ylabel("PC2")
    ax.grid(True, linestyle="--", alpha=0.2)
    plt.tight_layout()
    fig.savefig(os.path.join(out_dir, "05_pca_clusters_with_centers_status.png"), dpi=dpi)
    plt.close(fig)


# =========================
# DIAGNOSTICS (elbow + metrics)
# =========================
def save_diagnostics(
    ...
    # use only numerical part for silhouette/DB/CH
    X_num = X[:, :X.shape[1] - len(cat_indices)].astype(float)

    for kk in ks:
        kp = KPrototypes(n_clusters=kk, init=km_init, n_init=km_n_init, verbose=0)
...
def fit_kprototypes(
    ...
    kp = KPrototypes(n_clusters=cfg.k, init=cfg.km_init, n_init=cfg.km_n_init, verbose=0, gamma=cfg.gamma,)
    clusters = kp.fit_predict(X, categorical=cat_indices)
    return kp, clusters


def stabilize_labels_by_size(
    clusters: np.ndarray, k: int
) -> np.ndarray:
...
def main() -> None:
    ...
    # drop rows with missing in any used feature
    all_feats = list(cfg.num_features) + list(cfg.cat_features)
    df = df.dropna(subset=all_feats).reset_index(drop=True)
    logging.info("Total rows used (after dropna): %d", len(df))

    X, cat_indices, valid_idx, scaler = prepare_data(df, cfg.cat_features, cfg.num_features)
    X_num = X[:, :len(cfg.num_features)].astype(float)

    # diagnostics
    logging.info("Running diagnostics k=%d..%d", cfg.k_min, cfg.k_max)
    save_diagnostics(
        X=X, cat_indices=cat_indices,
        out_dir=cfg.base_dir, k_min=cfg.k_min, k_max=cfg.k_max,
        km_init=cfg.km_init, km_n_init=cfg.diag_n_init,
        dpi=cfg.dpi, figsize=cfg.diag_figsize,
    )

    # fit
    logging.info("Fitting KPrototypes k=%d", cfg.k)
    ...
    # cluster sizes
    save_cluster_sizes_plot(clusters, cfg.base_dir, cfg.k, cfg.dpi, cfg.diag_figsize)

    # PCA plot
    save_pca_plot(
        X_num=X_num,
        clusters=clusters,
        patstat=df_out[cfg.patstat_col].to_numpy(),
    ...
    # feature plots
    save_cat_distributions(df_out, cfg.base_dir, cfg.cat_features, cfg.dist_figsize, cfg.dpi)
    save_num_distributions(df_out, cfg.base_dir, cfg.num_features, cfg.dist_figsize, cfg.dpi)
    ...
    save_df(df_out, os.path.join(cfg.base_dir, "full_cluster_assignments.csv"))
    save_df(summary, os.path.join(cfg.base_dir, "full_cluster_summary.csv"), index=True)

    # evaluation
    logging.info("Evaluating internal metrics...")
    internal = evaluate_internal(X_num, clusters, cfg.silhouette_sample_size, cfg.random_state)

    logging.info("Evaluating stability (%d runs)...", cfg.stability_runs)
    stability = evaluate_stability(
        X, cat_indices, cfg.k, cfg.km_init, cfg.km_n_init,
    ...
    logging.info("Internal metrics: %s", internal)
    logging.info("Stability metrics: %s", stability)
